  ...
  input.forEach((element) => {
    ...
    if (element.value.length == 0) {
      ...
      if (element.name == "Password" && element.value.length < 6) {
        ...
        element.value = "";
        p.className = "error";
      } else if (element.name == "Email" && !email.value.match(validRegex)) {
        element.parentElement.className = "invalid";
        ...
        (element.name == "First Name" || element.name == "Last Name") &&
        !element.value.match(letters)
      ) {
        element.parentElement.className = "invalid";
        element.placeholder = "";
